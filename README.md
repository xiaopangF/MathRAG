...
|---|---:|---:|---:|---:|
| BGE Embedding + Reranker | 85.00% | 95.00% | 96.00% | 0.8987 |
| BM25 + BGE + Reranker | 88.00% | 97.00% | 99.00% | 0.9262 |

当前自动化测试基线：`80 passed, 3 skipped`。
...
无效数值、布尔值或日志级别会在启动时直接报错。`POST /api/settings/deepseek-key` 只修改当前后端进程，服务重启后失效；持久配置应写入 `.env`。
...
| `POST` | `/api/settings/deepseek-key` | 临时更新 DeepSeek Key |
...
这些运行数据默认不提交到 Git。旧索引可能没有页码字段；重新上传 PDF 并构建索引后才能获得完整页码引用。

## 已知限制

- PDF 公式和复杂排版的解析效果受 PyMuPDF 文本提取质量影响
- 检索效果依赖教材结构、切分质量和评测集覆盖度
- 页码定位在复杂排版下可能只能精确到页码范围
- 当前是本地单用户模型，没有账号、权限隔离和租户数据边界
- DeepSeek Key 临时设置接口没有生产级身份认证
