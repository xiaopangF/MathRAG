...
每个新知识库的 `processed/` 目录会保存 `pages.jsonl`、`full_text.txt` 和 `extraction_summary.json`。其中解析摘要包含文本页数、空白页、版面类型、被移除的页眉页脚数量，以及 OCR 建议、成功、失败和跳过的页码。
