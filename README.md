...
- 检索链路完整：BGE 向量召回 + BM25 词法召回 + 候选合并去重 + BGE Reranker 精排。
...
|---|---:|---:|---:|---:|
| 关键词命中 | 83.16% | 100.00% | 100.00% | 0.9123 |
| 页码命中 | 53.68% | 81.05% | 90.53% | 0.6784 |
| 章节命中 | 43.16% | 64.21% | 71.58% | 0.5432 |

说明：

- 总题数 100，其中 95 题可回答、5 题越界拒答。
- 关键词 Recall@5 已达到 100%，说明候选片段基本能召回到答案依据。
- 页码和章节 Recall 仍有明显优化空间，当前报告保留 9 个页码未命中和 27 个章节未命中案例，后续用于修切分器和 metadata 继承。
